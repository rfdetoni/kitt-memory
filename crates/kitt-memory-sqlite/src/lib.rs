...
impl MemoryStore for SqliteMemoryStore {
    fn remember(&self, memory: NewMemory) -> Result<MemoryRecord> {
        ...
        // Exact active duplicate: return existing record instead of growing storage.
        if let Some(existing) = self.with_conn(|conn| conn.query_row(
            "SELECT id FROM memories WHERE namespace=?1 AND workspace_id=?2 AND content_hash=?3 AND status='ACTIVE' LIMIT 1",
            params![record.namespace, record.workspace_id, record.content_hash], |r| r.get::<_,String>(0)).optional())? {
            return self.with_conn(|conn| load_one(conn, &existing))?.ok_or_else(|| MemoryError::Storage("duplicate lookup vanished".into()));
        }
        match self.upsert_record(&record) {
            ...
            Err(err) => {
                if let Some(existing) = self.with_conn(|conn| conn.query_row(
                    "SELECT id FROM memories WHERE namespace=?1 AND workspace_id=?2 AND content_hash=?3 AND status='ACTIVE' LIMIT 1",
                    params![record.namespace, record.workspace_id, record.content_hash], |r| r.get::<_,String>(0)).optional())? {
                    return self.with_conn(|conn| load_one(conn, &existing))?.ok_or(err);
                }
                Err(err)
            }
        }
    ...
    fn recall(&self, q: &RecallQuery) -> Result<Vec<MemoryRecord>> {
        ...
        let mut candidates = self.with_conn(|conn| {
            ...
            let ids = stmt.query_map(params![q.namespace,q.workspace_id], |r| r.get::<_,String>(0))?.collect::<std::result::Result<Vec<_>,_>>()?;
            let mut out=Vec::with_capacity(ids.len());
            for id in ids { if let Some(m)=load_one(conn,&id)? { out.push(m); } }
            Ok(out)
        })?;
        ...
        if !candidates.is_empty() {
            ...
            let _guard = self
                ...
                .lock()
                .map_err(|_| MemoryError::Storage("memory write lock poisoned".into()))?;
            self.with_conn(|conn| { let tx=conn.unchecked_transaction()?; for id in &ids { tx.execute("UPDATE memories SET last_accessed_at=?1, access_count=access_count+1 WHERE id=?2",params![now,id])?; } tx.commit()?; Ok(()) })?;
        }
        Ok(candidates)
    }
    ...
    fn consolidate_exact_duplicates(&self, namespace: &str, workspace_id: &str) -> Result<usize> {
        let rows=self.with_conn(|conn|{
            ...
            stmt.query_map(params![namespace,workspace_id],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?)))?.collect::<std::result::Result<Vec<_>,_>>()
        })?;
...
fn load_one(
    ...
    conn.query_row("SELECT id,namespace,workspace_id,kind,content,normalized_content,status,sensitivity,scope,importance,confidence,created_at,updated_at,last_accessed_at,access_count,valid_until,supersedes_id,content_hash,pinned,metadata_json FROM memories WHERE id=?1",[id],|r|Ok(MemoryRecord{
        id:r.get(0)?,namespace:r.get(1)?,workspace_id:r.get(2)?,kind:MemoryKind::from_db(&r.get::<_,String>(3)?),content:r.get(4)?,normalized_content:r.get(5)?,status:MemoryStatus::from_db(&r.get::<_,String>(6)?),sensitivity:Sensitivity::from_db(&r.get::<_,String>(7)?),scope:MemoryScope::from_db(&r.get::<_,String>(8)?),importance:r.get::<_,f64>(9)? as f32,confidence:r.get::<_,f64>(10)? as f32,created_at:r.get(11)?,updated_at:r.get(12)?,last_accessed_at:r.get(13)?,access_count:r.get::<_,i64>(14)? as u64,valid_until:r.get(15)?,supersedes_id:r.get(16)?,content_hash:r.get(17)?,pinned:r.get::<_,i64>(18)?!=0,metadata_json:r.get(19)?
    })).optional()
}
...
fn migrate(conn: &Connection) -> std::result::Result<(), rusqlite::Error> {
    ...
    CREATE TABLE IF NOT EXISTS memories(
      ...
      created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, last_accessed_at INTEGER, access_count INTEGER NOT NULL DEFAULT 0,
      valid_until INTEGER, supersedes_id TEXT, content_hash TEXT NOT NULL, pinned INTEGER NOT NULL DEFAULT 0, metadata_json TEXT NOT NULL DEFAULT '{}'
    );
...
mod tests {
    ...
    fn remembers_and_recalls() {
        let dir = std::env::temp_dir().join(format!("kitt-memory-test-{}", std::process::id()));
        let _ = std::fs::remove_file(&dir);
        let store = SqliteMemoryStore::open(&dir).unwrap();
        store
            .remember(NewMemory {
                namespace: "assistant".into(),
                workspace_id: "global".into(),
                kind: MemoryKind::UserPreference,
                content: "Prefere respostas curtas".into(),
                sensitivity: Sensitivity::Private,
